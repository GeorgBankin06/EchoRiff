...
object Constants {
    ...
    const val ADMIN_EMAIL = "georgio3383@gmail.com"
    const val API_TOKEN = ""
}

fun loadNavGraph(fragment: Fragment, graphId: Int) {
    // Clear the back stack before loading a new NavHostFragment
    fragment.parentFragmentManager.popBackStack(
        null,
        androidx.fragment.app.FragmentManager.POP_BACK_STACK_INCLUSIVE
    )

    // Create a new NavHostFragment instance with the required navigation graph
    ...
    // Apply custom animations to the fragment transaction
    ...
    // Replace the current NavHostFragment with the new one
    fragmentTransaction
...
fun fadeInHint(editText: EditText, fromColor: Int, toColor: Int, duration: Long = 500) {
    val colorAnimator = ValueAnimator.ofArgb(fromColor, toColor)
    colorAnimator.duration = duration
    colorAnimator.addUpdateListener { animator ->
        editText.setHintTextColor(animator.animatedValue as Int)
